...
/// Extract only canopy's own flags; pass everything else through to Claude Code.
fn parse_args() -> Args {
        ...
        if arg == "-h" || arg == "--help" {
            eprintln!(
                "A TUI file explorer for Claude Code CLI\n\n\
                 Usage: canopy [OPTIONS] [CLAUDE_ARGS...]\n\n\
                 Options:\n\
                 ...
                 Example: canopy --resume\n\
                 Example: canopy -p /my/project --continue"
            );
            std::process::exit(0);
        }
        if arg == "-V" || arg == "--version" {
            eprintln!("canopy {}", env!("CARGO_PKG_VERSION"));
            std::process::exit(0);
        }
